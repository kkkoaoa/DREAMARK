...
for epoch in {0..49..1}
do
    model="main/debiased_models/bert-large-uncased/gender_brief-neutral_epsilon_0_1/$epoch"
    ...
    CUDA_VISIBLE_DEVICES=5 nohup python -u stereoset.py \
        ...
        --algorithm DPCE \
        --batch_size 1 \
        > ../log/evaluate/evaluate_gender_brief-neutral_epsilon_0_1.log 2>&1 &
    wait
    echo ${model////_}
    wait
    nohup python -u stereoset_evaluation.py \
    --predictions_file "./results/stereoset/${model////_}.json" \
    > ../log/evaluate/res_gender_brief-neutral_epsilon_0_1.log 2>&1 &
done
