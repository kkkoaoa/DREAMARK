...
def parse_file(gold_file, predictions_file):
    ...
    if args.output_file:
        ...
        output_file = f"./results/stereoset/{args.predictions_file.split('/')[-1].split('.')[0]}_results.json"
...
if __name__ == "__main__":
    ...
    if args.predictions_dir is not None:
        ...
        for prediction_file in glob.glob(predictions_dir + "*.json"):
            ...
            parse_file(f"{args.persistent_dir}/data/stereoset/test.json", prediction_file)
    else:
        parse_file(f"{args.persistent_dir}/data/stereoset/test.json", args.predictions_file)
    
    os.remove(args.predictions_file)
    print(f"Remove {args.predictions_file}")
