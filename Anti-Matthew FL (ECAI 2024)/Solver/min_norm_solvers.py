...
class MinNormSolver(object):
    def __init__(self, n, eps_delta) -> None:
        self.MAX_ITER = 1000
        self.STOP_CRIT = 1e-5

        self.n = n  # the dimension of \theta
        self.eps_delta = eps_delta  # the error bar of the optimization process [eps1 < g, eps2 < delta1, eps3 < delta2]
    ...
    def _min_norm_element_from2(self, v1v1, v1v2, v2v2):
        ...
        gamma = (-1.0 * ((v1v2 - v2v2) / (v1v1 + v2v2 - 2 * v1v2))).item()
    ...
    def _min_norm_2d(self, vecs, dps):
        ...
        for i in range(len(vecs)):
            for j in range(i + 1, len(vecs)):
                ...
                c, d = self._min_norm_element_from2(
    ...
    def find_min_norm_element(self, vecs, deltas, factor_delta, lr_delta):
        """
        ...
        if n < 3:
            ...
            # for i in range(n):
            #     if (
            #         self.eps_delta[i] < deltas[i]
            #         and torch.linalg.norm(vecs[i]) * factor_delta <=  deltas[i]
            #     ):
            #         deltas[i] = lr_delta * deltas[i]
            return sol_vec, init_sol[2], deltas
...
def gradient_normalizers(grads, losses, normalization_type):
    ...
    if normalization_type == "l2":
        for i in range(len(grads)):
            gn.append(
                np.sqrt(
                    np.mean([gr.pow(2).sum().data.cpu() + 1e-12 for gr in grads[i]])
                )
            )
    elif normalization_type == "loss":
        for i in range(len(grads)):
            gn.append(losses[i].detach() + 1e-12)
    elif normalization_type == "loss+":
        for i in range(len(grads)):
            gn.append(
                (losses[i].detach() + 1e-12)
                * np.sqrt(np.mean([gr.pow(2).sum().data.cpu() for gr in grads[i]]))
            )
