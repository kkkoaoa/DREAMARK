...
def to_dual(A, x, b, c, eps, random_proj=False):
    """
        A: (D, N)
        x: (B, D)
        b: upper bound
...
class ImageWatermark:
    ...
    def __init__(self, orthn_basis=None, upper=0.9, lower=-0.9,
                 dims=[4,64,64],
                 device=torch.device('cuda')):
        self.dim = np.prod(dims)
        if orthn_basis is None:
            # compute orthonormal basis
            ortho_fn = Path(f"data/orthobasis_{self.dim}_{_dtype}.pt")
            if ortho_fn.exists():
