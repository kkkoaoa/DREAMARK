...
from .watermark import HiddenMark, ResidueMark
...
class Trainer(object):
    def __init__(self, 
        ...
        # watermark
        self.watermark = HiddenMark(opt, device=device, H=opt.h, W=opt.w, msg_length=48 if opt.msg_length is None else opt.msg_length, trigger_size=opt.trigger_size)
    ...
    def train_step(self, data):
        ...
        # if self.opt.msg_length:
        #     msg_length = self.watermark.msg.shape[-1]
        #     fts = self.watermark.decoder(pred_rgb)[:, :msg_length]
        #     msg_loss = _message_loss(fts=fts, targets=self.watermark.msg)
        #     loss += msg_loss
    ...
    def evaluate(self, loader, name=None):
        # self.watermark = ResidueMark(opt=self.opt, device=self.device, H=self.opt.h, W=self.opt.w, guidance=self.guidance)
        # if self.opt.init_ckpt != '':
        #     checkpoint_dict = torch.load(self.opt.init_ckpt, map_location=self.device)
        #     self.watermark.load_state_dict(checkpoint_dict)
        self.model.eval()
        # self.watermark.H = 800
        # self.watermark.W = 800
        # self.watermark.cx = 400
        # self.watermark.cy = 400
        acc, pred_rgbs = self.watermark.eval_step(self.model)
        torchvision.utils.save_image(pred_rgbs[0], os.path.join(self.workspace, "0.png"))

        noise_acc = []
        target_rgb = gaussian_blur(pred_rgbs[0], sigma=0.1)
        torchvision.utils.save_image(target_rgb, os.path.join(self.workspace, "gaussian_blur.png"))
        ac = self.watermark.eval_img(target_rgb)
        noise_acc.append(ac)

        target_rgb = gaussian_noise(pred_rgbs[0], sigma=0.1)
        torchvision.utils.save_image(target_rgb, os.path.join(self.workspace, "gaussian_noise.png"))
        ac = self.watermark.eval_img(target_rgb)
        noise_acc.append(ac)

        # target_rgb = jpeg_compress(pred_rgbs[0], 10)
        # torchvision.utils.save_image(target_rgb, os.path.join(self.workspace, "jpeg_compression.png"))
        # ac = self.watermark.eval_img(target_rgb)
        # noise_acc.append(ac)

        target_rgb = rotate(pred_rgbs[0], 30)
        torchvision.utils.save_image(target_rgb, os.path.join(self.workspace, "rotate.png"))
        ac = self.watermark.eval_img(target_rgb)
        noise_acc.append(ac)

        target_rgb = adjust_brightness(pred_rgbs[0], 2)
        torchvision.utils.save_image(target_rgb, os.path.join(self.workspace, "brightness.png"))
        ac = self.watermark.eval_img(target_rgb)
        noise_acc.append(ac)

        target_rgb = center_crop(pred_rgbs[0], 0.4)
        torchvision.utils.save_image(target_rgb, os.path.join(self.workspace, "center_crop.png"))
        ac = self.watermark.eval_img(target_rgb)
        noise_acc.append(ac)
        print(acc, '\n', noise_acc)
    ...
    def train_one_epoch(self, loader):
        ...
        for data in loader:
            ...
            if self.opt.msg_length:
                wm_loss = self.watermark.train_step(self.model)
                loss += wm_loss
    ...
    def evaluate_one_epoch(self, loader, name=None, shading = None):
        ...
        self.model.eval()
        if self.opt.eval_mark and shading=='albedo':
            acc, pred_rgbs = self.watermark.eval_step(self.model)
            self.log(f"Acc={acc}")
