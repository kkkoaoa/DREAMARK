...
class NeRFRenderer(nn.Module):
    def __init__(self, opt, device):
        ...
        # self.latent_mode = opt.latent_mode

        # if not self.latent_mode:
        #     import raymarching
        #     self.image_channel = 3
        # else:
        #     import raymarchinglatent as raymarching
        ...
        if self.opt.dmtet:
            ...
            self.verts = torch.tensor(tets['vertices'], dtype=torch.float32, device=device) * 2 # covers [-1, 1]
            ...
            edges = torch.tensor([0,1, 0,2, 0,3, 1,2, 1,3, 2,3], dtype=torch.long, device=device) # six edges for each tetrahedron.
    ...
    def run(self, rays_o, rays_d, light_d=None, ambient_ratio=1.0, shading='albedo', bg_color=None, perturb=False, **kwargs):
        ...
        # nears.unsqueeze_(-1)
        # fars.unsqueeze_(-1)
        nears, fars = near_far_from_bound(rays_o, rays_d, self.bound, type='cube' if self.opt.cube else 'sphere', min_near=self.min_near)
        ...
        rgbs = rgbs.view(N, -1, self.img_channels) # [N, T+t, 3]
        ...
        image = image.view(*prefix, self.img_channels)
