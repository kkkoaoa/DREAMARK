...
class RandomDiffJPEG(AugmentationBase2D):
    def __init__(self, p, low=10, high=100, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')) -> None:
        super().__init__(p=p)
...
class HiddenAug(nn.Module):
    ...
    def __init__(self, img_size, p_crop=0.3, p_blur=0.3, p_jpeg=0.3, p_rot=0.3, p_color_jitter=0.3, p_res=0.3, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
        super().__init__()
        augmentations = []
...
class KorniaAug(nn.Module):
    def __init__(self, degrees=30, crop_scale=(0.2, 1.0), crop_ratio=(3/4, 4/3), blur_size=17, color_jitter=(1.0, 1.0, 1.0, 0.3), diff_jpeg=10,
                ...
                cropping_mode='slice', img_size=224, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')
            ):
