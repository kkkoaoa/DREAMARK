...
from nerf import Trainer
from nerf.attack.finetune import FTAL
from nerf.attack.pruning import Pruning
...
if __name__ == '__main__':
    ...
    parser.add_argument('--trigger_size', default=100, type=int)
    parser.add_argument('--msg_length', default=None, type=int)
    parser.add_argument('--eval_mark', action='store_true')
    ...
    if opt.msg_length:
        opt.workspace += f'-{opt.msg_length}'
    if opt.latent == True:
    ...
    model = NeRFNetwork(opt, device).to(device)
    ...
    print(model)
    ori_model = NeRFNetwork(opt, device).to(device)
    ...
    if opt.test:
        ...
        # trainer = Pruning(' '.join(sys.argv), 'df', opt, model, guidance, device=device, workspace=opt.workspace, optimizer=optimizer, ema_decay=None, fp16=opt.fp16, lr_scheduler=scheduler, use_checkpoint=opt.ckpt, eval_interval=opt.eval_interval, scheduler_update_every_step=True, ori_model=ori_model)
        trainer = FTAL(' '.join(sys.argv), 'df', opt, model, guidance, device=device, workspace=opt.workspace, optimizer=optimizer, ema_decay=None, fp16=opt.fp16, lr_scheduler=scheduler, use_checkpoint=opt.ckpt, eval_interval=opt.eval_interval, scheduler_update_every_step=True, ori_model=ori_model)
