...
if __name__ == "__main__":
    ...
    axs[0].set_ylabel("Sampling size", fontsize=16)
    axs[0].set_title("$U_{EO}$", loc="center", fontsize=16)

    axs[1].set_xlabel("Iterations", fontsize=16)
    # axs[1].set_ylabel("Sampling size", fontsize=16)
    axs[1].set_title("$U_{DP}$", loc="center", fontsize=16)
    ...
    predict_value = copy.deepcopy(clf.predict)
    predict_value2 = copy.deepcopy(predict_value)
    ...
    (
        ...
        new_dataset.copy(), 0.1, 0.1, 1e-3, 1e-3, predict_value2, z_index=z_index
    )
    axs[1].plot(
    ...
    (
        ...
        new_dataset.copy(), 0.1, 0.1, 1e-3, 1e-3, predict_value, z_index=z_index
    )
    axs[0].plot(
        list(range(1, len(min_miu_record) + 1)),
        [_ * 10000 for _ in real_miu],
    ...
    plt.savefig("fig/miu_min_iterate_DP.jpg")
    plt.savefig("fig/miu_min_iterate_DP.pdf")
    plt.savefig("fig/miu_min_iterat_DP.svg", format="svg")
    print("end")
