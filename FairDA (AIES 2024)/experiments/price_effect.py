...
def scipy_solve(
    ...
    constraint1 = lambda mu_further: abs((sum([a_1[i] * mu_further[i] for i in range(len(mu_further))]) + c_1) / (sum([a_2[i] * mu_further[i] for i in range(len(mu_further))]) + c_2)-(sum([a_3[i] * mu_further[i] for i in range(len(mu_further))]) + c_3) / (sum([a_4[i] * mu_further[i] for i in range(len(mu_further))]) + c_4))
    ...
    bounds = Bounds([0.0 for i in range(len(a_0))],[1.0-mu_min[i] for i in range(len(a_0))])
    ...
    res = differential_evolution(objective, bounds=bounds, constraints=nlc, tol=1e-7,disp=False)
    # Print the result
    return res.x
    # except:
    #     print("None")
...
if __name__ == "__main__":
    ...
    clf=[LogisticRegression(random_state=0), MLPClassifier(random_state=0), MLPClassifier(random_state=0)]
    for kk in range(0,3):
        ...
        (
            ...
            D_test_x,
            D_test_y,
        ) = func[kk](D_init_num=1, D_providers_num=5, add_noise_bool=True)

        D_init =  np.concatenate((D_init_x[0], D_init_y[0]), axis=1)
        D_provider = []
        providers_num = 5
        for ii in range(providers_num):
            D_provider.append(np.concatenate((D_provider_x[ii], D_provider_y[ii]), axis=1))
        ...
        for budget in budgets:    
            ...
            print ("test before data acquisiton:  test_accuracy:",accuracy_test,"   test_dp:", dp)
            # calculate miu_min
            D_size=[]
            D_size_z1=[]
            D_size_z0=[]
            ...
            for ii in range(providers_num):
                ...
                miu_pz1_record,
                miu_pz0_record,
            ) = IterativeCal(D_provider[ii], 0.1, 0.1, 1e-3, 1e-3, clf[kk].predict, z_index[kk])
                ...
                if subset.shape[0]<int(min_samplingratio * D_provider[ii].shape[0]):
                    selected_rows = np.random.choice(
                        ...
                        size=int(min_samplingratio * D_provider[ii].shape[0])-subset.shape[0],
                        replace=True,
                    )
                    c_subset =remained_dataset[selected_rows]
                    remained_dataset = np.delete(remained_dataset, selected_rows, axis=0)
                ...
                D_size_z1.append(D_provider[ii].shape[0]*(np.count_nonzero(
                    D_mined[ii][:, z_index[kk]] >0
                ) )/(np.count_nonzero(
                    D_mined[ii][:, z_index[kk]] >0
                )+np.count_nonzero(
                    D_mined[ii][:, z_index[kk]] <=0
                ...
                D_size_z0.append(D_provider[ii].shape[0]*(np.count_nonzero(
                    D_mined[ii][:, z_index[kk]] <=0
                ) )/(np.count_nonzero(
                    D_mined[ii][:, z_index[kk]] >0
                )+np.count_nonzero(
            ...
            mu_continue=scipy_solve( D_size,
            Ua_list,
            pz1_list,
            D_size_z1,
            pz0_list,
            ...
            print ("test after data acquisiton:  test_accuracy:",accuracy_test,"   test_dp:", dp)   
        np.save("record/epsilon_unbias_test_accuracy_before"+str(kk)+".npy",np.array(test_accuracy_before)) 
        np.save("record/epsilon_unbias_test_dp_before"+str(kk)+".npy",np.array(test_dp_before)) 
        np.save("record/epsilon_unbias_test_accuracy_after"+str(kk)+".npy",np.array(test_accuracy_after)) 
        np.save("record/epsilon_unbias_test_dp_after"+str(kk)+".npy",np.array(test_dp_after)) 
        print("end")
