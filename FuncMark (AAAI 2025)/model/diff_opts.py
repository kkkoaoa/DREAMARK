...
def hessian(y, x, create_graph=True):
    ...
    h = torch.zeros(*(y.shape), x.shape[-1], x.shape[-1]).to(y.device)
...
def jacobian(y, x, create_graph=False):
    """
    Jacobian of y wrt x
    y: shape (B, N, channels)
    x: shape (B, N, dim)
    ret: shape (B, N, channels, dim)
    """
    jac = torch.zeros(
        *(y.shape), x.shape[-1]).to(y.device)
    for i in range(y.shape[-1]):
        # calculate dydx over batches for each feature value of y
        y_flat = y[...,i].view(-1, 1)
        jac[..., i, :] = grad(
            y_flat, x, torch.ones_like(y_flat), create_graph=create_graph, retain_graph=True)[0]
