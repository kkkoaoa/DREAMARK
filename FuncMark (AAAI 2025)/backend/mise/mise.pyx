...
"""
      /- - - - /
     /        /|  每个voxel都用A的坐标标记, 并且被一个idx唯一标记
    /- - - - / |    idx = \vec{A} \cdot (R^2, R, 1)
    |       |  |  其中A为离原点最近的gridpoint, 并且被一个idx唯一标记
    |       | /     idx = \vec{A} \cdot (R^2, R, 1)
    A- - - -|/

    从A映射到voxel object的路径:
            A ---vec_to_idx---> idx ---MISE.voxels---> voxel
    从A映射到gridpoint object的路径:
            A ---vec_to_idx---> idx ---MISE.grid_point_hash---> grid_points_vector_idx ---grid_points---> gridpoint
"""
