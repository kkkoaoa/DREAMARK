...
# import model.pointconv.pytorch_utils as pt_utils
...
class ComputeVertexNormals(Function):
    ...
    def forward(ctx, xyz, faces):
        """
            ...
            faces: (B, M, 3)
        """
        normlized_normals, normals, norms = _ext.compute_vertex_normals(xyz, faces)
        ctx.for_backwards = (xyz, faces, normals, norms)
        return normlized_normals
    @staticmethod
    def backward(ctx, grad_out):
        """
            grad_out: (B, N, 3)
        """
        xyz, faces, normals, norms = ctx.for_backwards
        return _ext.compute_vertex_normals_grad(grad_out.contiguous(), xyz, faces, normals, norms), None

compute_vertex_normals = ComputeVertexNormals.apply

class BuildEdgeMatrixFromTriangle(Function):
    @staticmethod
    def forward(ctx, xyz, faces):
        """
            xyz: (B, N, 3)
            faces: (B, M, 3)
        """
        out = _ext.build_edge_matrix_from_triangle(xyz, faces)
...
build_edge_matrix_from_triangle = BuildEdgeMatrixFromTriangle.apply

class BuildGraphFromImage(Function):
    @staticmethod
    def forward(ctx, img):
        """
            img: (B, 3, H, W)
        """
        xyz, idx = _ext.build_graph_from_img(img)
        ctx.mark_non_differentiable(xyz)
...
build_graph_from_img = BuildGraphFromImage.apply
...
class GraphNeighborQuery(Function):
    ...
    def forward(ctx, xyz, faces, nsample):
        """
            ...
            faces: (B, M, 3)
        """
        idxs, num = _ext.graph_neighbor_query(xyz, faces, nsample)
        ctx.mark_non_differentiable(idxs)
        ctx.mark_non_differentiable(num)
        return idxs, num
...
class GraphNeighborQueryAll(Function):
    @staticmethod
    def forward(ctx, xyz, faces):
        """
            xyz: (B, N, 3)
            faces: (B, M, 3)
        """
        idxs, num = _ext.graph_neighbor_query_all(xyz, faces)
        ctx.mark_non_differentiable(idxs)
        ctx.mark_non_differentiable(num)
        return idxs, num
...
graph_neighbor_query_all = GraphNeighborQueryAll.apply
...
class QueryAndGroup(nn.Module):
    ...
    def forward(self, xyz, new_xyz, features):
        ...
        new_features = grouping_operation(features, idx)
