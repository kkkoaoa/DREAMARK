...
"""
This file is adapted from I-Chun Chern et al.'s work.
Original repository: https://github.com/GAIR-NLP/factool
...
class code_pipeline(pipeline):
    ...
    async def run_with_tool_api_call(self, prompts, responses, entry_points):
        ...
        for i in range(num_batches):
            ...
            responses_returned = await self.run_with_tool_live(self.sample_list[batch_start: batch_end],
    ...
    async def run_with_tool_dataset(self, annotated_dataset_path: str, with_tool_classified_dataset_path: str,
        ...
        for i in range(num_batches):
            ...
            for j, response in enumerate(responses):
                ...
                self.sample_list[index]['with_tool_classification'] = response[
                    'with_tool_classification'] if response is not None else 'None'
    ...
    async def run_self_check_live(self, fewshot, batch):
        ...
        messages_list = [
            [
                ...
                {"role": "user",
                 "content": self.self_check_prompt[user_prompt_key].format(input_question=response['prompt'],
                                                                           input_solution=response['completion'])},
            ]
    ...
    async def run_self_check_dataset(self, annotated_dataset_path: str, self_check_classified_dataset_path: str,
        ...
        for i in range(num_batches):
            ...
            for j, response in enumerate(responses):
                ...
                self.sample_list[index]['self_check_classification'] = response.get('factuality',
                                                                                    'None') if response is not None else 'None'
                self.sample_list[index]['self_check_reasoning'] = response.get('reasoning',
                                                                               'None') if response is not None else 'None'
